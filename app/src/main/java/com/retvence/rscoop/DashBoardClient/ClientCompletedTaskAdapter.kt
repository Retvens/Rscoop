...
class ClientCompletedTaskAdapter(val context: Context, var userList:List<GetTaskData>) : RecyclerView.Adapter<ClientCompletedTaskAdapter.ClientCompletedTaskViewHolder>() {

    var adapterPosition = -1
    var favourite = false

    class ClientCompletedTaskViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ClientCompletedTaskViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.recentcard,parent,false)
        return ClientCompletedTaskViewHolder(view)
    }
